...
class PAT_Save_Controller {
		...
		if ( ! $overall_success ) {
			...
			foreach ( $successful_rows as $successful_row ) {
				$history_changes = isset( $successful_row['_pat_history_changes'] ) && is_array( $successful_row['_pat_history_changes'] )
					...
					: array();
				$history_context = isset( $successful_row['_pat_history_context'] ) && is_array( $successful_row['_pat_history_context'] )
					? $successful_row['_pat_history_context']
					: array();
	...
	private function save_row( $row, int $index ): array {
		...
		return $this->build_result(
			...
			array(
				...
				'_pat_history_context' => $this->build_history_context( $row_type, $normalized, isset( $service_result['data'] ) && is_array( $service_result['data'] ) ? $service_result['data'] : array(), absint( $result_id ) ),
	...
	/**
	 * Build extra history context for later undo/ledger behavior.
	 *
	 * @param array<string, mixed> $normalized_row
	 * @param array<string, mixed> $service_data
	 * @return array<string, mixed>
	 */
	private function build_history_context( string $row_type, array $normalized_row, array $service_data, int $row_id ): array {
		$context = array();

		if ( 'variation' === $row_type && ! empty( $normalized_row['is_generated'] ) && ! empty( $service_data['is_created'] ) ) {
			$context['undo_strategy']          = 'delete_created_variation';
			$context['created_from_generated'] = true;
			$context['client_row_id']          = isset( $normalized_row['client_row_id'] ) ? (string) $normalized_row['client_row_id'] : (string) $row_id;
			$context['parent_id']              = isset( $service_data['parent_id'] ) ? absint( $service_data['parent_id'] ) : ( isset( $normalized_row['parent_id'] ) ? absint( $normalized_row['parent_id'] ) : 0 );
		}
