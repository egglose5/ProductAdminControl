...
<?php if ( $has_rows ) : ?>
	<?php foreach ( $rows as $child ) : ?>
		...
		$child_row_id       = '' !== $child_id_raw ? $child_id_raw : $child_temp_id;
		$child_parent_id    = isset( $child['parent_id'] ) ? (int) $child['parent_id'] : ( isset( $parent_id ) ? (int) $parent_id : 0 );
		...
		$child_attributes   = is_string( $child_attributes ) ? $child_attributes : '{}';
		?>
		<tr class="pat-row pat-child-row is-child-row is-hidden<?php echo $is_generated ? ' is-generated-row' : ''; ?>" data-pat-parent="<?php echo esc_attr( $parent_dom_id ); ?>" data-pat-parent-id="<?php echo esc_attr( $child_parent_id ); ?>" data-pat-row-id="<?php echo esc_attr( $child_row_id ); ?>" data-pat-row-type="variation" data-pat-generated="<?php echo $is_generated ? 'true' : 'false'; ?>" data-pat-temp-id="<?php echo esc_attr( $child_temp_id ); ?>" data-pat-generated-attributes="<?php echo esc_attr( $child_attributes ); ?>" <?php echo $is_generated ? 'data-pat-row-state="dirty"' : ''; ?> tabindex="0" aria-selected="false" hidden>
			<td class="check-column">
				<input type="checkbox" data-pat-select-row="true" aria-label="<?php echo esc_attr( sprintf( __( 'Select variation row %s', 'product-admin-tool' ), $child_row_id ) ); ?>" />
			</td>
			<td class="pat-cell-product">
				...
				<div class="pat-row-meta">
					<span class="pat-row-id"><?php echo esc_html( sprintf( __( 'ID: %s', 'product-admin-tool' ), $child_row_id ) ); ?></span>
			...
			<td>
				...
				<input
					id="<?php echo esc_attr( 'pat-variation-sku-' . $child_row_id ); ?>"
			...
			<td class="pat-price-cell">
				<div class="pat-inline-field-group">
					<label class="screen-reader-text" for="<?php echo esc_attr( 'pat-variation-regular-price-' . $child_row_id ); ?>"><?php esc_html_e( 'Variation regular price', 'product-admin-tool' ); ?></label>
					<input
						id="<?php echo esc_attr( 'pat-variation-regular-price-' . $child_row_id ); ?>"
				...
				<div class="pat-inline-field-group">
					<label class="screen-reader-text" for="<?php echo esc_attr( 'pat-variation-sale-price-' . $child_row_id ); ?>"><?php esc_html_e( 'Variation sale price', 'product-admin-tool' ); ?></label>
					<input
						id="<?php echo esc_attr( 'pat-variation-sale-price-' . $child_row_id ); ?>"
						class="pat-inline-field pat-inline-price"
						type="number"
						step="0.01"
						inputmode="decimal"
			...
			<td>
				<label class="screen-reader-text" for="<?php echo esc_attr( 'pat-variation-stock-' . $child_row_id ); ?>"><?php esc_html_e( 'Variation stock quantity', 'product-admin-tool' ); ?></label>
				<input
					id="<?php echo esc_attr( 'pat-variation-stock-' . $child_row_id ); ?>"
			...
			<td>
				<label class="screen-reader-text" for="<?php echo esc_attr( 'pat-variation-status-' . $child_row_id ); ?>"><?php esc_html_e( 'Variation status', 'product-admin-tool' ); ?></label>
				<select
					id="<?php echo esc_attr( 'pat-variation-status-' . $child_row_id ); ?>"
			...
			<td>
				<div class="pat-inline-field-group">
					<label class="screen-reader-text" for="<?php echo esc_attr( 'pat-variation-menu-order-' . $child_row_id ); ?>"><?php esc_html_e( 'Variation menu order', 'product-admin-tool' ); ?></label>
					<input
						id="<?php echo esc_attr( 'pat-variation-menu-order-' . $child_row_id ); ?>"
			...
			<td class="pat-row-state">
				<span class="pat-row-state-message" data-pat-row-state-message><?php echo $is_generated ? esc_html__( 'Ready to create', 'product-admin-tool' ) : ''; ?></span>
			</td>
		</tr>
	<?php endforeach; ?>
